...
\nonumber
\end{align}
At a given $(k,m)$ write $\mathsf{g}_i\triangleq\Vert\vec{h}_{i,k,m}\Vert^2$ and $c_{n,i}\triangleq|\vec{h}_{n,k,m}^{\sf H}\vec{h}_{i,k,m}|^2$. Let $m(n)$ be the UAV serving UE $n$, $\mathcal{N}_m^{\tt ul}=\{n:m(n)=m\}$, $\mathcal{N}_k^{\tt sub}=\{i:a_{i,k}^{\tt ul}=1\}$, and $\mathcal{W}_{n,k,m}=\{i\in\mathcal{N}_m^{\tt ul}\cap\mathcal{N}_k^{\tt sub}\setminus\{n\}:\mathsf{g}_i\le\mathsf{g}_n\}$ the weaker intra-cell co-channel users. The received vector at UL UAV $m$ on sub-channel $k$ is
\begin{equation}
\begin{aligned}
...
\label{eq:ul_y}
\end{equation}
with UAV-to-UAV channel $\vec{G}_{m',k,m}\in\mathbb{C}^{L\times L}$, jammer activity $a_{q,k}\sim\mathrm{Bernoulli}(q_k)$ ($q_k$ is a probability, distinct from the DL powers $q_{m',n'}$), and $\vec{z}_{k,m}\sim\mathcal{CN}(\vec{0},N_0\vec{I}_L)$. MF-SIC uses $\vec{v}_{m,n}=\vec{h}_{n,k,m}$ and cancels stronger users first. The SIJNR is
\begin{equation}
\gamma_{n,k,m}
...
$\Xi_{k,m}=\sum_{m'\in\mathcal{M}_k^{\tt sub}}\Vert\vec{G}_{m',k,m}\vec{P}_{m',k}^{\tt UAV}\Vert^2$ is the power radiated on sub-channel $k$ by the DL UAVs $\mathcal{M}_k^{\tt sub}$ with transmit matrices $\vec{P}_{m',k}^{\tt UAV}$, and
$\mathcal{J}_{n,k,m}=\sum_{q\in\mathcal{Q}}a_{q,k}c_{n,q} p_q$ is jamming, $\mathcal{Q}$ being the jammer set. Because the MF combiner is unnormalized, the projected noise is $\mathsf{g}_nB_kN_0$ and the isotropic DL leakage is $\mathsf{g}_n\Xi_{k,m}/L$, so both scale with $\mathsf{g}_n$ as the numerator does.
...
\label{eq:sic}
\end{equation}
If $\mathcal{W}_{n,k,m}=\emptyset$ (the cell-head), the constraint is vacuous.
The rate is $R_{n,k,m}=B_k\log_2(1+\gamma_{n,k,m})$, and $a_n^{\tt ul}=\sum_k a_{n,k}^{\tt ul}$ with $\rho_n\le a_n^{\tt ul}$ so that a UE without a sub-channel computes locally. The local branch and the offload branch run in parallel, so with $R_n\triangleq\sum_{m,k}a_{n,k}^{\tt ul}R_{n,k,m}$ and ignoring result-feedback payload,
\begin{align}
...
\label{eq:Theta}
\end{align}
where $\xi_n\in(0,1]$ is the power-amplifier efficiency. $\Theta_n$ is UE-side energy only (local compute and UL transmit); UAV server energy is omitted.
...
\label{sec:dl}
DL UAV $m'$ uses one sub-channel, $\sum_k a_{m',k}^{\tt dl}=1$, and ZFBF $\vec{W}_{m',k}=\vec{H}_{m',k}^{\tt dl}[(\vec{H}_{m',k}^{\tt dl})^{\sf H}\vec{H}_{m',k}^{\tt dl}]^{\dagger}$, the Moore--Penrose inverse, which nulls intra-cell interference when $L\ge|\mathcal{N}_{m'}^{\tt dl}|$ and is regularized otherwise. The SIJNR of UE $n'$ is
\begin{equation}
\textstyle\gamma_{m',k,n'}^{\tt dl}=q_{m',n'}/(\mathcal{I}_{n'}^{\tt inter}+\mathcal{I}_{n'}^{\tt CCI}+\mathcal{J}_{n'}^{\tt dl}+\sigma_{k,n'}^2),
\label{eq:dl_sijnr}
\end{equation}
where $\mathcal{I}_{n'}^{\tt inter}=\sum_{j\neq m'}\sum_{i\in\mathcal{N}_j^{\tt dl}}q_{j,i}|\vec{h}_{n',k,j}^{\sf H}\vec{w}_{j,k,i}|^2$ is inter-cell DL interference,
...
Then $R_{n'}^{\tt dl}=\sum_{m',k}a_{m',k}^{\tt dl}B_k\log_2(1+\gamma_{m',k,n'}^{\tt dl})$, with $\sum_{n'}q_{m',n'}\Vert\vec{w}_{m',n'}\Vert^2\le P_{m'}^{\tt max}$ and $\gamma_{m',k,n'}^{\tt dl}\ge\gamma_{\tt th}$.

\section{Problem and Solution}
\label{sec:sol}
The UL utility now carries a third, sensing-derived term,
\begin{equation}
\mathbb{U}_n^{\tt ul}
...
\label{eq:Uul}
\end{equation}
with $\beta_n^{\tt t}+\beta_n^{\tt e}+\beta_n^{\tt a}=1$, and the DL utility is $\mathbb{U}_{n'}^{\tt dl}=R_{n'}^{\tt dl}/R_{\tt const}$ with $R_{\tt const}$ a reference rate that puts both utilities on one scale. The snapshot objective is $\mathbb{U}=\sum_{n\in\mathcal{N}^{\tt ul}}\mathbb{U}_n^{\tt ul}+\sum_{n'\in\mathcal{N}^{\tt dl}}\mathbb{U}_{n'}^{\tt dl}$. The HPPP-averaged utility is
\begin{equation}
\Upsilon_{\tt sys}
=\mathbb{E}[\mathbb{U}].
\label{eq:upsilon}
...
&\gamma_{m',k,n'}^{\tt dl}\ge\gamma_{\tt th},\eqref{eq:sic}\forall n\in\mathcal{N}_k^{\tt sub},\forall m,k.\label{eq:c-sic}
\end{align}
\end{subequations}
...
\label{eq:Fbar}
\end{equation}
is the rate beyond which the offload branch of \eqref{eq:branches} falls below the local branch (with $\bar F_{nm}=\infty$ when the bracket is nonpositive), and $\mu_m>0$ is the unique water level giving $\sum_{n\in\mathcal{N}_m^{\tt ul}}F_{nm}^\star=F_m^{\tt max}$, or $\mu_m\to0$ if $\sum_n\bar F_{nm}\le F_m^{\tt max}$.
\end{proposition}
\begin{IEEEproof}
...
\label{eq:rhostar}
\end{equation}
at which the two branches of \eqref{eq:branches} balance. An optimizer lies in $\{0,\rho_n^\star,1\}$, so $\rho_n^\diamond$ follows from three utility evaluations.
\end{proposition}
\begin{IEEEproof}
...
\begin{figure}[!t]
\centering
\subfloat[]{\includegraphics[width=0.49\linewidth]{figures/compare_with_ex_su.png}}
\hfill
\subfloat[]{\includegraphics[width=0.49\linewidth]{figures/compare_with_ex_time.png}}
...
\begin{figure}[!t]
\centering
\subfloat[]{\includegraphics[width=\linewidth]{figures/ue_density_su.png}}
\\
\subfloat[]{\includegraphics[width=\linewidth]{figures/uav_density_su.png}}
...
\begin{figure}[!t]
\centering
\subfloat[]{\includegraphics[width=0.49\linewidth]{figures/accuracy_tradeoff.png}}
\hfill
\subfloat[]{\includegraphics[width=0.49\linewidth]{figures/retention_vs_snr.png}}
