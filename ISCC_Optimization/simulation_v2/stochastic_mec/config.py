"""Configuration objects for the SI-TNTN UAV-MEC ISCC simulator (manuscript_v2)."""
...
class SystemParams:
    ...
    # MF-SIC post-combining noise/co-channel scaling.  v1 used L B N0 and the raw
    # DL->UL power Xi, which is off by ||h||^2 / L against the p ||h||^4 numerator
    # (UL SNR ~ -50 dB, so nothing ever offloaded).  True uses ||h||^2 B N0 and the
    # isotropic MF projection ||h||^2 Xi / L.  False reproduces the v1 SINR.
...
class AlgorithmParams:
    ...
    # bisection accuracy for the quasi-convex lower bound of Theorem 3
