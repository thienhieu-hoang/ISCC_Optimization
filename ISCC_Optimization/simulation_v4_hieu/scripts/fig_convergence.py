...
def _save_curve_data(curves: dict[str, np.ndarray], path: Path) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    payload = {k: np.asarray(v).tolist() for k, v in curves.items()}
    path.write_text(json.dumps(payload, indent=2))
    print(f"saved {path}")
    try:
        from scipy.io import savemat
        mat_path = path.with_suffix(".mat")
        mat_payload = {k.replace("-", "_"): np.asarray(v) for k, v in curves.items()}
        savemat(mat_path, mat_payload)
        print(f"saved {mat_path}")
    except Exception as exc:
        print(f"[warning] could not save .mat: {exc}")


def run_convergence(args) -> None:
    params = SystemParams()
    ...
    res_path, fig_path = outputs(args, "convergence_bwoa")
    _save_curve_data(outer, res_path)
    ...
    res2, fig2 = outputs(args, "convergence_mpc")
    _save_curve_data(inner, res2)
    if plotting is not None:
